...
import { View, Text, StyleSheet, ScrollView, Image, Modal, TouchableHighlight } from 'react-native';
...
const getCards = (groupId:number, filter:string) => new Promise((resolve, reject)=>{
    ...
    const sql = !groupId
      ? `select * from cards ${filter==='byName'?'order by nameGroup':'order by companyGroup'}` 
      : `select * from cards where groupId=${groupId} ${filter==='byName'?'order by nameGroup':'order by companyGroup'}`
          ...
          <View style={styles.modalView}>
           ...
           <WalletHeader onPressOut={() => navigation.push('WalletSearch')}/>
            <View style={styles.menu}>
              <TouchableOpacity style={styles.menuGroup} onPressOut={() => navigation.navigate('SelectGroup', {onGoBack1: setGroupId, onGoBack2: setCurrentGroup, currentGroupId: groupId})}>
            <Text>{currentGroup}</Text>
              ...
              <View style={styles.menuExtra}>
                ...
                onPressOut={() => {
                  ...
                  navigation.push('EditCard', {cards: cardsData, setMainToggle:setToggle, mainToggle:toggle})
                  }}>
                  <Text>편집</Text>
