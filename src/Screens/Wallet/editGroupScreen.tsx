...
const EditGroupScreen = ({navigation, route}: Props) => {
  ...
  const [selectedGroupId, setSelectedGrouId] = useState(0);
            ...
            <View style={styles.modalView}>
              <View>
                ...
                <View style={{flexDirection: 'row'}}>
                  <Text style={{fontFamily: 'sd_gothic_b', color: '#7D7D7D'}}>
                    전체삭제 선택 시 그룹 안의{' '}
                  </Text>
                  <Text style={{fontFamily: 'sd_gothic_b', color: '#EA6060'}}>
                    모든 명함들이 삭제
                  </Text>
                  <Text style={{fontFamily: 'sd_gothic_b', color: '#7D7D7D'}}>
                    됩니다.
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    db.deleteGroupAndCards(deleteGroupId).then(() => {
                      ...
                      setModalVisible2(!modalVisible2);
                      let newDeleted = deleted.slice();
                      newDeleted.push(selectedGroupId);
                      route.params.setDeletedGroupId(newDeleted);
                    });
                  }}>
            ...
            <View style={{...styles.modalView, height: '28%'}}>
              ...
              <View style={{flexDirection: 'row', justifyContent: 'center'}}>
                ...
                <TouchableOpacity
                  style={{...styles.openButton, backgroundColor: '#6078EA'}}
                  onPress={() => {
                    db.getCountByGroupId(deleteGroupId).then(
                      (cntForDelete: any) => {
        ...
        <View style={styles.content}>
          ...
          {groups.map((group: any) => {
            if (groups.length === 1) {
              ...
              return (
                <TextInputForGroupEdit
                  setDeleteGroupId={setDeleteGroupId}
                  onPressDeleteBtn={() => {
                    setSelectedGrouId(group.id);
