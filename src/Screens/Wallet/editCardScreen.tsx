...
const updateGroupCnt = (idList:any) => new Promise((resolve, reject) => {
...
const deleteCards = (checkList:any) => new Promise((resolve, reject) => {
    const db = SQLite.openDatabase(
        {
          name: 'mat.db',
          location: 'Library',
...
const moveGroup = (checkList:any, newGroupId:number) => new Promise((resolve, reject) => {
  const db = SQLite.openDatabase(
    {
      name: 'mat.db',
      location: 'Library',
    ...
    return (
        ...
        onPress={() => {
            ...
            <View style={styles.left}>
                <Image source={checked? imgChecked: imgUnchecked} style={styles.imgChecked} />
            ...
            <View style={styles.itemCardContainer}>
                <WalletCard cardWidth={cardWidth} data={JSON.parse(fullData)} />
            </View>
        </TouchableOpacity>
    )
}
    ...
    return (
        <View>
            ...
            <View style={styles.content}>
                <ScrollView>
                    {cards.map((card:any) => <CardItem key={card.id} card={card} setCheckList={setCheckList} checkList={checkList} />)}
                </ScrollView>
            </View>
            ...
            <View style={styles.footerMenu}>
                ...
                onPress={() => {
                  ...
                  navigation.push('MoveGroup', 
                  {
                    ...
                    mainToggle: route.params.mainToggle
                        ...
                        <Image source={imgPeople} style={{width: 30, height: 17, marginHorizontal: 10}} />
                        <Text>그룹 이동</Text>
                ...
                <TouchableOpacity style={styles.footerItem}
                onPress={() => {
                    const groupIds = cards.filter( (card:any) => checkList.includes(card.id)).map((card:any) => card.groupId)
                    updateGroupCnt(groupIds).then(() => console.log('updated!'))
                    
                    deleteCards(checkList).then(() => {
                        setCards( cards.filter((card:any) => !checkList.includes(card.id)) )
